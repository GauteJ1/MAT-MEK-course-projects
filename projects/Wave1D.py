...
class Wave1D:
    ...
    def D2(self, bc):
        ...
        if bc["left"] == 0:
            # Use matrix for dirchlet instead of specifying later
            D[0, :2] = 0, 0
        if bc["right"] == 0:
            D[-1, -2:] = 0, 0
    ...
    def apply_bcs(self, bc, u=None):
        ...
        if bc["left"] == 2:  # Open boundary
            u[0] = (
                2 * (1 - self.cfl) * self.un[0]
                - (1 - self.cfl) / (1 + self.cfl) * self.unm1[0]
                + 2 * self.cfl**2 / (1 + self.cfl) * self.un[1]
            )
        if bc["right"] == 2:
            u[self.N] = (
                2 * (1 - self.cfl) * self.un[self.N]
                - (1 - self.cfl) / (1 + self.cfl) * self.unm1[self.N]
                + 2 * self.cfl**2 / (1 + self.cfl) * self.un[self.N - 1]
            )

        if bc["left"] == 3 and bc["right"] == 3:  # Periodic has to be on both sides
            u[self.N] = u[0]
